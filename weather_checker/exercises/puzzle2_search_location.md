...
เพื่อความเป็นระเบียบ เราจะสร้างแต่ละ tool ในไฟล์แยก โดย export function ที่รับ `server` แล้วลงทะเบียน tool:
...
แล้ว import + เรียกใช้ใน `src/index.ts`:
```typescript
...
| `___BLANK_3___` | API endpoint | `"search"` |
| `___BLANK_4___` | ข้อความนำหน้ารายการ | `"ผลการค้นหาตำแหน่ง:\n"` |
