...
export function registerGetForecast(server) {
    server.registerTool("get_forecast", {
        description: "ดูพยากรณ์อากาศล่วงหน้าหลายวัน",
        inputSchema: {
            // latitude คือ ละติจูด เช่น 13.75
            latitude: z.number().describe("latitude คือ ละติจูด เช่น 13.75"),
            // longitude คือ ลองจิจูด เช่น 100.5
            longitude: z.number().describe("longitude คือ ลองจิจูด เช่น 100.5"),
            // days คือ จำนวนวันที่ต้องการพยากรณ์ (1-16)
            days: z.number().min(1).max(16).describe("days คือ จำนวนวันที่ต้องการพยากรณ์ (1-16)"), // ใส่คำอธิบาย parameter เช่น "จำนวนวันที่ต้องการพยากรณ์ (1-16)"
        },
