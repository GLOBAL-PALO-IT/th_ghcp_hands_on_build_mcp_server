...
export function registerGetForecast(server: McpServer) {
    server.tool(
        ...
        "ดูพยากรณ์อากาศล่วงหน้าหลายวัน",
        {
            ...
            days: z.___BLANK_1___().min(1).max(16).describe("จำนวนวันที่ต้องการพยากรณ์ (1-16)"), // ใส่ชนิดข้อมูล Zod เช่น "number"
        },
        async ({ latitude, longitude, days }) => {
            ...
            const forecastList = data.daily.time
                .map((date, i) => {
                    ...
                    const rain = data.daily.___BLANK_2___[i]; // ใส่ชื่อ property สำหรับปริมาณฝน เช่น "precipitation_sum"
